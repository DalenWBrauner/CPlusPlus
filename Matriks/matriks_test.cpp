...
void test_inv06()
{
	...
	vector<vector<float>> o = {{0, 1, 4}, {1, 0, -3}, {2, 3, 8}};
	matriks<float> original = o;
	vector<vector<float>> i = {{-9.0/2.0, -2.0, 3.0/2.0}, {7, 4, -2}, {-3.0/2.0, -1, 1.0/2.0}};
	matriks<float> inverse = i;
	matriks<float> inverse_attempt = original.inverse();

	cout << "original:\t(plugged in manually)\n";
	cout << original << "\n";
	cout << "inverse:\t(plugged in manually)\n";
	cout << inverse << "\n";
	cout << "inverse_attempt:\t(generated by .inverse())\n";
	cout << inverse_attempt << "\n";

	if (inverse.isInverseOf(original))
		{ cout << "inverse is Inverse of original\n"; }
	if (inverse_attempt.isInverseOf(original))
		{ cout << "inverse_attempt is Inverse of original\n"; }
